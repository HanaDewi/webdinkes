...
class PencapaianController extends Controller
{
    public function pencapaian(Request $request): Response
    {
        ...
        $keg_filter = $request->keg;
        $apbd_filter = $request->apbd;
...
// dd($pencapaians);
        ...
        return response()->view('pencapaian.pencapaian', compact('pencapaians', 'tahun', 'keg', 'apbd', 'tahun_filter', 'keg_filter', 'apbd_filter', 'bulan_filter'));
    }
    ...
    public function subprogram(Request $request)
    {
        ...
        if ($request->has('keg')) {
            $query->where('keg', $keg_filter);
        }
        if ($request->has('apbd')) {
            $query->where('apbd', $apbd_filter);
        }
...
// Check if the final realization exceeds 100
        if ($realisasi_akhir > 100) {
            return redirect()->route('pencapaian.subprogram', [
                'tahun' => $validateData['tahun'],
                'keg' => $validateData['keg'],
                'apbd' => $validateData['apbd'],
                'bulan' => $validateData['bulan'],
            ])->with('failed', 'Realisasi melebihi 100');
        } else {
